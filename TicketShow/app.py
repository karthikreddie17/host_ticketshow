...
class AdminLoginForm(FlaskForm):
    adminname = StringField('Admin Name', validators=[InputRequired(), Length(min=4, max=50)], render_kw={"placeholder": "Username"})
    password = PasswordField('Password', validators=[InputRequired(), Length(min=4, max=50)], render_kw={"placeholder": "Password"})
    submit = SubmitField('Login')

class UserLoginForm(FlaskForm):
    username = StringField('User Name',  validators=[InputRequired(), Length(min=4, max=50)], render_kw={"placeholder": "Username"})
    password = PasswordField('Password', validators=[InputRequired(), Length(min=4, max=50)], render_kw={"placeholder": "Password"})
    submit = SubmitField('Login')
...
def book_tickets():
    if request.method == 'POST':
        ...
        search_results = Venues.query.filter(or_(
            Venues.venue_name.ilike(f"%{search_term}%"),
            Shows.show_name.ilike(f"%{search_term}%"),
            Shows.show_tags.ilike(f"%{search_term}%"),
            Venues.venue_location.ilike(f"%{search_term}%")
        )).join(Shows).all()
